      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">
          <span className="bg-gradient-to-r from-[#ffa649] to-[#ff4c88] bg-clip-text text-transparent">
            {translate('Medical Treatments')}
          </span>
        </h1>
        <p className="text-muted-foreground mt-2">
          {translate('Explore world-class healthcare procedures available across Delhi NCR.')}
        </p>
      </div>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4 my-4 bg-gradient-to-r from-[#ffa649]/5 to-[#ff4c88]/5 p-4 rounded-xl border border-[#ffa649]/10">
              <div>
                <span className="text-xs text-muted-foreground flex items-center gap-1.5 mb-1">
                  <Clock className="w-3.5 h-3.5 text-[#ffa649]" /> {translate('Recovery Timeline')}
                </span>
                <p className="text-sm font-medium">{translate(selectedTreatment.recoveryTime)}</p>
              </div>
              <div>
                <span className="text-xs text-muted-foreground flex items-center gap-1.5 mb-1">
                  <Activity className="w-3.5 h-3.5 text-[#ffa649]" /> {translate('Hospital Stay Required')}
                </span>
                <p className="text-sm font-medium">{translate(selectedTreatment.hospitalStay)}</p>
              </div>
            </div>
            ...
            <div className="mb-6">
              <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                ...
                <span className="bg-gradient-to-r from-[#ffa649] to-[#ff4c88] bg-clip-text text-transparent">
                  {translate('Financial Estimates')}
                </span>
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 border border-[#ffa649]/10 rounded-lg bg-card">
                  <span className="text-xs text-muted-foreground block mb-0.5">{translate('Domestic Cost (INR)')}</span>
                  <p className="text-sm font-medium text-foreground">
                    {translate('Starting')}: <span className="text-[#ffa649] font-semibold">₹{selectedTreatment.startingCostINR.toLocaleString("en-IN")}</span>
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {translate('Avg. Package')}: ₹{selectedTreatment.averageCostINR.toLocaleString("en-IN")}
                  </p>
                </div>
                <div className="p-3 border border-[#ffa649]/10 rounded-lg bg-card">
                  <span className="text-xs text-muted-foreground block mb-0.5">{translate('International Cost (USD)')}</span>
                  <p className="text-sm font-medium text-foreground">
                    {translate('Starting')}: <span className="text-[#ff4c88] font-semibold">${selectedTreatment.startingCostUSD.toLocaleString("en-US")}</span>
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {translate('Avg. Package')}: ${selectedTreatment.averageCostUSD.toLocaleString("en-US")}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-[#ffa649]/10 pt-5">
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
                  <Building2 className="w-3.5 h-3.5 text-[#ffa649]" /> {translate('Recommended Centers (Delhi NCR)')}
                </h4>
                <ul className="space-y-1.5 text-sm">
                  {selectedTreatment.recommendedHospitals.map((hospital, index) => (
                    <li key={index} className="text-foreground/90 flex items-start gap-1">
                      <span className="text-[#ffa649] font-bold">•</span> {translate(hospital)}
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
                  <UserCheck className="w-3.5 h-3.5 text-[#ff4c88]" /> {translate('Panel Specialists')}
                </h4>
                <ul className="space-y-1.5 text-sm">
                  {selectedTreatment.recommendedDoctors.map((doctor, index) => (
                    <li key={index} className="text-foreground/90 flex items-start gap-1">
                      <span className="text-[#ff4c88] font-bold">•</span> {translate(doctor)}
                    </li>
                  ))}
                </ul>
              </div>
